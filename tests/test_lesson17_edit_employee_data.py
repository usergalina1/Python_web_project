...
from api.test_lesson16_17_api_requests import Api
...
class EditEmployeeDataTests(AdminUserAuthentication):
    ...
    def setUp(self):
        ...
        api = Api()
        api.authenticate()
        result = api.add_employee(firstName=self.emp.first_name, lastName=self.emp.last_name, emp_id=emp_id,
    ...
    def test_edit_employee_middle_nick_name(self):
        ...
        emp_details_page = PersonalDetailsPageDay13(self.browser)
        emp_details_page.edit()
        emp_details_page.update_personal_details(middle_name=middle_name, nick_name=nick_name)
        self.assertIn(middle_name, emp_details_page.get_profile_pic_header())
        self.assertEqual(f'{self.emp.first_name} {middle_name} {self.emp.last_name}',
                         emp_details_page.get_profile_pic_header())
